...
### 1. 物理隔离与代码安全 (2026-01-28 更新)
*   **代码托管选型**：正式环境**严禁使用 Gitee/Coding** 等国内平台。国内平台必须实名，且会对代码内容进行敏感词扫描，存在溯源风险。
*   **GitHub 匿名托管**：使用海外邮箱（Outlook/Gmail）注册 GitHub 账号，无需实名。将所有代码（前端+后端）推送到 GitHub **Private Repository (私有仓库)**，既能享受 Vercel 的自动部署，又能实现内容隐蔽。
*   **Vercel 联动**：在 Vercel 中直接关联 GitHub 私有仓库，实现“代码推送 -> 全球部署”的自动化链路。
